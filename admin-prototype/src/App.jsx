...
function NavGroups({ groups, path, isActive, closedGroups, toggleGroup, onNavigate }) {
  return <div className="nav-scroll">{groups.map(([group, items]) => <div className="nav-group" key={group}><button className="nav-group-toggle" aria-expanded={!closedGroups.has(group)} onClick={()=>toggleGroup(group)}><span>{group}</span><ChevronDown size={13} className={closedGroups.has(group)?"closed-arrow":""}/></button><div className={`nav-group-items ${closedGroups.has(group)?"closed":""}`}><div>{items.map(([label,to,Icon,badge]) => <button title={label} key={to} className={isActive(to,path)?"active":""} onClick={()=>onNavigate(to)}><Icon size={18}/><span>{label}</span>{badge && <span className="nav-badge">{badge}</span>}</button>)}</div></div></div>)}</div>;
}
...
function RecentPaymentsCard({go}) { return <Card><div className="card-head"><div><h2>Recent payments</h2></div><button onClick={()=>go("/my-salary")}>View all <ChevronRight size={15}/></button></div>{myPayments.map(([name,sub,amount,status])=><div className="activity-item" key={name}><span><CircleDollarSign size={18}/></span><div><strong>{name}</strong><small>{sub}</small></div><Status>{status}</Status><b>{amount}</b></div>)}</Card> }
function UpcomingCard() { return <Card><div className="card-head"><h2>Upcoming</h2></div>{myUpcoming.map(([name,date,eta,Icon])=><div className="activity-item" key={name}><span><Icon size={18}/></span><div><strong>{name}</strong><small>{date}</small></div><small>{eta}</small></div>)}</Card> }
...
function MyLeavesPage({open}) {
  ...
  return <>
    ...
    <div className="tabs pill-tabs in-card">{["All Leaves","Pending","Approved","Rejected"].map(x=><button key={x} className={tab===x?"active":""} onClick={()=>setTab(x)}>{x}<span>{tabCounts[x]}</span></button>)}</div>
...
function LeavesPage({open}) {
  ...
  return <><PageTitle className="leave-page-title" title="Leave Management" subtitle="Review and manage employee leave requests" actions={<><Button kind="secondary" icon={CalendarDays}>Leave Sheet</Button><Button icon={Plus} onClick={()=>open("leave")}>Apply for Employee</Button></>}/><div className="tabs-scroll"><div className="tabs pill-tabs">{["Pending","Completed","All Requests"].map(x=><button key={x} className={tab===x?"active":""} onClick={()=>changeTab(x)}>{x}<span>{x==="Pending"?pending:x==="Completed"?allRows.length-pending:allRows.length}</span></button>)}</div></div><Card><Toolbar><SearchBox placeholder="Search employee" value={q} onChange={setQ}/><Select value={typeFilter} onChange={setTypeFilter}><option>All leave types</option>{leaveTypeOptions.map(t=><option key={t}>{t}</option>)}</Select><Select value={state} onChange={setState}><option>Data</option><option>Empty</option><option>Loading error</option></Select><Button kind="secondary" icon={Filter} onClick={()=>setShowFilters(v=>!v)}>Filters{activeFilterCount>0&&<b className="filters-count-badge">{activeFilterCount}</b>}</Button></Toolbar>{showFilters&&<div className="leave-filters-panel"><Select value={teamFilter} onChange={setTeamFilter}><option>All teams</option>{teamOptions.map(t=><option key={t}>{t}</option>)}</Select>{tab!=="Pending"&&<Select value={statusFilter} onChange={setStatusFilter}><option>All statuses</option>{statusFilterOptions.map(s=><option key={s}>{s}</option>)}</Select>}<button type="button" className={`changes-only-toggle ${overlapOnly?"active":""}`} onClick={()=>setOverlapOnly(v=>!v)} title="Show only requests that overlap with another teammate's leave"><Check size={14}/>Overlapping only</button><div className="activity-filter-field"><span>Start date</span><input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)}/></div><div className="activity-filter-field"><span>End date</span><input type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)}/></div><div className="activity-filter-field"><span>&nbsp;</span><button type="button" className="link filters-clear-link" onClick={clearFilters}>Clear filters</button></div></div>}{tab==="Pending"&&selected.size>0&&<div className="bulk-action-bar"><strong>{selected.size} selected</strong><Button icon={Check} onClick={()=>decide(rows.filter(r=>selected.has(r.id)),"Approved")}>Approve</Button><Button kind="danger" icon={X} onClick={()=>decide(rows.filter(r=>selected.has(r.id)),"Rejected")}>Reject</Button><button className="bulk-clear" onClick={()=>setSelected(new Set())}>Clear</button></div>}{state==="Loading error"?<ErrorState message="The leave request service returned an unexpected database response."/>:state==="Empty"||!rows.length?<Empty title="No leave requests" text={state==="Empty"?`There are no ${tab.toLowerCase()} leave requests.`:"No leave requests match your search and filters."}/>:<DataTable columns={["Employee","Team","Leave type","Dates","Hours","Flags","Status"]} rows={cells} selectable={tab==="Pending"} selected={selected} onToggle={toggleOne} onToggleAll={toggleAll} getKey={(row,i)=>rows[i].id} renderActions={(r,i)=>{const rec=rows[i]; return <div className="row-actions"><IconButton icon={Eye} label="View request" onClick={()=>open("leave-detail",rec)}/>{rec.status==="Pending"&&<><IconButton icon={Check} label="Approve" success onClick={()=>decide([rec],"Approved")}/><IconButton icon={X} label="Reject" danger onClick={()=>decide([rec],"Rejected")}/></>}</div>}}/>}</Card></> }
...
function ActivityLogsPage() {
  ...
  return <><PageTitle title="Activity Logs" subtitle="Every action across the system — who did what, to whom, and when."/><div className="activity-filters"><div className="activity-filters-row"><SearchBox placeholder="Search by person, action, or resource…"/><FilterMenu value={action} options={ACTIVITY_ACTIONS} counts={actionCounts} onChange={setAction}/><button className={`filters-toggle ${moreFilters?"active":""}`} onClick={()=>setMoreFilters(v=>!v)}><SlidersHorizontal size={15}/>More filters{activeFilterCount>0&&<b>{activeFilterCount}</b>}</button></div><div className={`activity-filters-row activity-filters-secondary ${moreFilters?"open":""}`}>
    ...
    <div className="activity-filter-field"><span>Start date</span><input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)}/></div>
    <div className="activity-filter-field"><span>End date</span><input type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)}/></div>
