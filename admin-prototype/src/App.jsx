...
function Dashboard({ go, open, role }) {
  ...
  return <>
    <div className="tabs pill-tabs dashboard-view-tabs">{["Overview","My Space"].map(x=><button key={x} className={tab===x?"active":""} onClick={()=>setTab(x)}>{T(x)}</button>)}</div>
...
function MyDashboardContent({role,go}) {
  ...
  return <><PageTitle className="greeting-title" title={<>{T("Good morning, ")}<span>{isLead?"Ethan":"Matilda"}</span></>} subtitle={T("Here’s what’s happening at MUST today — 12 August 2026.")} actions={<Button kind="secondary" icon={FileText} onClick={()=>go("/my-salary")}>{T("View payslip")}</Button>}/><div className="metric-grid three employee-metrics"><Card className="simple-metric"><span>{T("Hourly rate")}</span><strong>$1.50</strong><p>{T("per hour")}</p></Card><Card className="simple-metric"><span>{T("Latest payslip")}</span><strong>$588.00</strong><p>{T("June 2026 · 392 hrs")}</p></Card><Card className="simple-metric"><span>{T("Tenure")}</span><strong>1y 4m</strong><p>since Feb 26, 2025</p></Card></div><div className="employee-dashboard-grid">{isLead?<Card className="attention-card"><div className="card-head"><div><h2><span className="count-badge">2</span>{T("Requests awaiting your approval")}</h2></div><button onClick={()=>go("/approvals")}>{T("View all")} <ChevronRight size={15}/></button></div>{[["SG","Sophie Grant","Annual Leave","12–14 Aug · 3 days","2 overlapping teammates"],["FH","Felix Harper","Overtime","14 Aug · 2 hrs",""]].map(([ini,name,type,date,overlap])=><button className="approval-row" key={name} onClick={()=>go("/approvals")}><Avatar initials={ini} small/><span><strong>{name}</strong><small>{type} · {date}</small>{overlap&&<em>{overlap}</em>}</span><b>{T("Review")}</b></button>)}<div className="active-request"><strong>{T("Your active request")}</strong><span>{T("Annual Leave · 12–14 Aug")}</span><Status>Pending</Status></div></Card>:<ActivityCard go={go}/>}<AttentionPanel go={go}/></div><div className="employee-dashboard-grid lower"><TeamAvailability/><div><Card className="holiday-card"><small>{T("NEXT PUBLIC HOLIDAY")}</small><h2>{T("Independence Day")}</h2><p>{T("Thu, 14 August · office closed")}</p><span>{T("2 days to go")}</span></Card><LeaveBalanceCard go={go}/></div></div><div className="employee-dashboard-grid lower"><RecentPaymentsCard go={go}/><UpcomingCard/></div></>;
}
...
const T=useT();
  ...
  return <>
    ...
    <div className="tabs pill-tabs in-card">{[T("All Leaves"),T("Pending"),T("Approved"),T("Rejected")].map(x=><button key={x} className={tab===x?"active":""} onClick={()=>setTab(x)}>{x}<span>{tabCounts[x]}</span></button>)}</div>
...
const T=useT();
  ...
  return <>
    ...
    <div className="tabs pill-tabs">{[T("Awaiting decision"),T("Decision history")].map(x=><button key={x} className={tab===x?"active":""} onClick={()=>setTab(x)}>{x}{x==="Awaiting decision"&&<span>{pendingCount}</span>}</button>)}</div>
...
const T=useT();
  ...
  return <><PageTitle eyebrow="" title={T("Organization Chart")} subtitle={T(isHierarchy?`${countHierarchy(ORG_HIERARCHY)} people in the reporting line`:`9 departments · ${teams.length} teams`)}/>
    <div className="org-live-toolbar">
      ...
      {!isHierarchy && <Select value={deptFilter} onChange={setDeptFilter}><option>{T("All departments")}</option>{DEPT_ORDER.map(d=><option key={d}>{d}</option>)}</Select>}
...
function ModalContent({type, data, role}) {
  ...
  if (type==="leave-type") return <div className="form-grid"><Field label={T("Leave type name")} required/><Field label={T("Allowance")} required><input type="number"/></Field><Field label={T("Unit")} required><Select><option>{T("Days")}</option><option>{T("Hours")}</option></Select></Field><Field label={T("Paid leave")}><Select><option>{T("Yes")}</option><option>{T("No")}</option></Select></Field><Field label={T("Minimum duration")}><input type="number"/></Field><Field label={T("Maximum duration")}><input type="number"/></Field><Field label={T("Eligibility")}><Select><option>{T("All employees")}</option><option>{T("After probation")}</option></Select></Field><Field label={T("Carry over")}><Select><option>{T("Not allowed")}</option><option>{T("Allowed")}</option></Select></Field><Field label={T("Description")} full><textarea/></Field></div>;
  ...
  if (type==="announcement") return <div className="form-grid"><Field label={T("Title")} required full><input placeholder={T("e.g. Public holiday on Monday")}/></Field><Field label={T("Audience")} required><Select><option>{T("All employees")}</option><option>{T("Specific department")}</option><option>{T("Specific team")}</option></Select></Field><Field label={T("Pin to top")}><Select><option>{T("No")}</option><option>{T("Yes")}</option></Select></Field><Field label={T("Message")} required full><textarea placeholder={T("Write the announcement…")}/></Field></div>;
  if (type==="feedback-cycle") return <div className="form-grid"><Field label={T("Cycle name")} required/><Field label={T("Feedback form")} required><Select><option>{T("Performance Review")}</option><option>{T("Probation Review")}</option></Select></Field><Field label={T("Start date")} required><input type="date"/></Field><Field label={T("End date")} required><input type="date"/></Field><Field label={T("Audience")} required><Select><option>{T("All employees")}</option><option>{T("Selected teams")}</option></Select></Field><Field label={T("Reviewers")}><Select><option>{T("Manager and self")}</option><option>{T("360 feedback")}</option></Select></Field><Field label={T("Allow negative ratings")}><Select><option>{T("No")}</option><option>{T("Yes")}</option></Select></Field><Field label={T("Lock responses after end date")}><Select><option>{T("Yes")}</option><option>{T("No")}</option></Select></Field></div>;
  ...
  if (type==="terminate") return <><div className="warning-box"><AlertTriangle size={20}/><p><strong>{T("End Matilda Ipeh Anashie’s employment?")}</strong> {T("Their HR history remains available.")}</p></div><div className="form-grid"><Field label={T("Last working day")} required><input type="date"/></Field><Field label={T("Reason")} required><Select><option value="">{T("Select a reason")}</option><option>{T("Resignation")}</option><option>{T("Termination")}</option><option>{T("Redundancy")}</option></Select></Field><Field label={T("Access end")} full><Select><option>{T("At end of last working day")}</option><option>{T("Immediately")}</option></Select></Field></div></>;
