...
const roleIdentity = {
  "Super Admin": { name:"Erwin Llanera", initials:"EL", title:"Recruitment Team Lead", department:"Human Resources Dep.", teamName:"Talent Acquisition Team", team:"HR-TAD", entity:"MUST Company PH", manager:"Ismail Gorkem Kara", country:"Philippines", phone:"+63 917 000 0001", email:"erwin.llanera@must.company", start:"Jan 6, 2023", tenure:"3y 7m", schedule:"Mon–Fri (5-day)", rate:"$6.25/hr", dob:"Mar 12, 1990", increment:"Jan 6, 2027", gender:"Male", employmentStatus:"Regular", probationEnd:"Apr 6, 2023", timezone:"Asia/Manila", linkedin:"www.linkedin.com/in/erwin-llanera", stack:["HR","Recruitment","Reports","HR Automation","Data Analysis","HRIS","AI & System Design"] },
  "Admin": { name:"Sneha Gupta", initials:"SG", title:"People Operations Manager", department:"Human Resources Dep.", teamName:"HR Management Team", team:"HR-TAD-HRM", entity:"MUST Company IN", manager:"Erwin Llanera", country:"India", phone:"+91 98200 00021", email:"g.sneha@must.company", start:"Apr 3, 2025", tenure:"1y 4m", schedule:"Mon–Sat (6-day)", rate:"$4.09/hr", dob:"May 27, 1995", increment:"Apr 3, 2027", gender:"Female", employmentStatus:"Regular", probationEnd:"Jul 3, 2026", timezone:"Asia/Kolkata", linkedin:"www.linkedin.com/in/sneha-gupta-hris", stack:["HR","Recruitment","Reports","HR Automation","Data Analysis","HRIS","AI & System Design"] },
  "Team Lead": { name:"Ethan Walker", initials:"EW", title:"Product Design Team Lead", department:"Blockchain Dp.", teamName:"UX/UI Team", team:"BLK-UXI", entity:"MUST SG", manager:"Ismail Gorkem Kara", country:"Ghana", phone:"+233 24 000 0011", email:"ethan.walker@must.company", start:"Jan 14, 2024", tenure:"2y 7m", schedule:"Mon–Fri (5-day)", rate:"$5.40/hr", dob:"Nov 2, 1992", increment:"Jan 14, 2027", gender:"Male", employmentStatus:"Regular", probationEnd:"Apr 14, 2024", timezone:"17:00 ~ 02:00 (KST)", linkedin:"www.linkedin.com/in/ethan-walker", stack:["Maze","Visual Studio Code","GitHub","Adobe Illustrator","Rive","Figma"] },
  "Employee": { name:"Matilda Ipeh Anashie", initials:"MA", title:"Senior Product Designer", department:"Blockchain Dp.", teamName:"UX/UI Team", team:"BLK-UXI", entity:"MUST SG", manager:"Ismail Gorkem Kara", country:"Ghana", phone:"+233 24 000 0000", email:"matilda.anashie@must.company", start:"Feb 26, 2025", tenure:"1y 4m", schedule:"Mon–Fri (5-day)", rate:"$1.50/hr", dob:"Oct 9, 1998", increment:"Feb 26, 2027", gender:"Female", employmentStatus:"Regular", probationEnd:"May 26, 2025", timezone:"17:00 ~ 02:00 (KST)", linkedin:"www.linkedin.com/in/matilda-anashie", stack:["Maze","Visual Studio Code","GitHub","Adobe Illustrator","Rive","Figma"] },
};
...
function ProfileField({icon:Icon,label,value,href}) { const T=useT(); return <div className={href?"self-profile-field is-link":"self-profile-field"}>{Icon&&<span><Icon size={18}/></span>}<div><small>{label}</small>{href?<a href={href} target="_blank" rel="noopener noreferrer">{value}</a>:<strong>{typeof value==="string"?T(value):value}</strong>}</div></div>; }
...
const T=useT();
  ...
  return <div className="self-profile">
    ...
    <Card className="self-profile-hero"><Avatar initials={person.initials}/><div><h1>{person.name}</h1><p>{T(person.title)} · {T(person.department)}</p><span><Status>{T("Active")}</Status><b>{T(person.employmentStatus)}</b></span></div></Card>
    <Card className="self-profile-section"><h2>{T("Personal information")}</h2><div className="self-profile-grid"><ProfileField icon={Mail} label={T("Email")} value={person.email}/><ProfileField icon={Phone} label={T("Phone")} value={person.phone}/><ProfileField icon={Building2} label={T("Department")} value={person.department}/><ProfileField icon={Building2} label={T("Company entity")} value={person.entity}/><ProfileField icon={BriefcaseBusiness} label={T("Designation")} value={person.title}/><ProfileField icon={CalendarDays} label={T("Joining date")} value={person.start}/><ProfileField icon={Clock3} label={T("Tenure")} value={person.tenure}/><ProfileField icon={BriefcaseBusiness} label={T("Work schedule")} value={person.schedule}/><ProfileField icon={CircleDollarSign} label={T("Hourly rate")} value={person.rate}/><ProfileField icon={Cake} label={T("Date of birth")} value={person.dob}/><ProfileField icon={TrendingUp} label={T("Next increment")} value={person.increment}/><ProfileField icon={IdCard} label={T("Gender")} value={person.gender}/><ProfileField icon={BriefcaseBusiness} label={T("Employment status")} value={person.employmentStatus}/><ProfileField icon={CalendarDays} label={T("Probation end")} value={person.probationEnd}/><ProfileField icon={Clock3} label={T("Time zone")} value={person.timezone}/><ProfileField label={T("LinkedIn")} value={person.linkedin} href={`https://${person.linkedin}`}/></div><div className="skill-list"><small>{T("Tech stack")}</small><div>{person.stack.map(x=><span key={x}>{x}</span>)}</div></div></Card>
    ...
    <Card className="self-profile-section"><h2>{T("Team & reporting")}</h2><div className="reporting-copy"><span>{T("Department & team")}</span><strong>{T(person.department)} <ChevronRight size={14}/> {T(person.teamName)} <b>{person.team}</b></strong></div><ProfileField icon={Users} label={T("Reports to")} value={person.manager}/>{isLead&&<button className="profile-team-link" onClick={()=>go("/my-team")}>{T("View your team")} <ChevronRight size={15}/></button>}</Card>
